...
5.  **成長:** 経験値を得てレベルアップし、より強力な装備を収集してパーティを強化する。
6.  **帰還:** 目的を達成したり、インベントリが一杯になったりしたら街に帰還し、次の探索に備える。
...
-   **ジョブ変更:** 作成後のジョブ変更は不可。
-   **編集:** 名前変更、装備/所持品の管理、ガンビット編集へ遷移できる。
...
| :--- | :--- | :--- | :--- |
| 味方のHP < 30% | HPが最も低い味方 | `大回復魔法` | 1 (最高) |
| 味方のHP < 70% | HPが最も低い味方 | `小回復魔法` | 2 |
| 敵が攻撃予兆 | 自分自身 | `防御する` | 3 |
...
-   自分自身の状態 (例: `自分のHP < 50%`)
-   味方の状態 (例: `味方の誰かのHP < 30%`)
-   敵の状態 (例: `敵の数が3体以上`, `敵が「攻撃予兆」状態`)
-   敵の種類 (例: `「非定型」の敵がいる`)
-   その他 (例: `常に`)

#### 2.3.2 ターゲット (Target) の種類
-   自分自身
-   味方 (例: `HPが最も低い味方`, `前衛の味方`)
-   敵 (例: `HPが最も低い敵`, `最も近い敵`, `自分を攻撃している敵`, `「非定型」の敵`)
...
### 2.5 ダンジョン
-   **自動生成:** フロアは自動で生成される。
-   **探索:** 自動で進行し、ログに `敵と遭遇した` `宝箱を見つけた` `罠にかかった` などのイベントが記録される。
...
5.  **キャラクター詳細画面:** ステータス、装備、インベントリ、ガンビットの確認・変更画面への遷移。
...
1.  **タイトル画面**
    -   **コンポーネント:** `New Game` / `Load Game` ボタン。
    -   **状態:** 選択中のセーブスロット (将来対応)。
2.  **街画面 (メインメニュー)**
...
3.  **酒場 (Tavern) 画面**
    ...
    -   **状態:** 選択中キャラクター。
4.  **パーティ編成画面**
    -   **コンポーネント:** 前衛/中衛/後衛スロット、キャラクター一覧。
    -   **状態:** スロット割り当て、選択中キャラクター。
5.  **キャラクター詳細画面**
    -   **コンポーネント:** ステータス表示、装備スロット、所持品リスト、`ガンビット編集` への導線。
    -   **状態:** 選択中装備/アイテム。
6.  **ガンビット編集画面**
...
8.  **ダンジョン選択画面**
    ...
    -   **状態:** 選択中フロア。
9.  **ダンジョン探索画面**
    -   **コンポーネント:** パーティ簡易ステータス、探索ログ、`Pause/Play`、`Camp`、`Return to City`。
    -   **状態:** `isPaused`、現在フロア、戦闘回数、ログバッファ。
10. **キャンプ画面**
    -   **コンポーネント:** 全回復ボタン、装備/ガンビット導線。
    -   **状態:** 特になし (回復後は探索へ戻る)。
...
4.  **戦闘ロジックの実装:** 1体の敵との戦闘をシミュレートし、ガンビットに従って行動が自動で実行され、ログが出力されることを確認する。
5.  **ダンジョン探索ループ:** ダンジョンに入り、自動で敵と遭遇し、戦闘が発生するサイクルを実装する。
6.  **成長と報酬:** 戦闘勝利時に経験値とアイテムドロップが発生し、キャラクターが成長する仕組みを実装する。
