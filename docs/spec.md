...
### 2.2 戦闘システム
-   **形式:** 時間進行型（ATB的）バトル。
-   **進行:** 各アクターの`speed`（素早さ）とアクション所要時間に応じて行動が並行して進行し、進行度が100%になった時点で効果が発動する。
-   **同時完了:** 複数の行動が同時に完了した場合は、`speed` が高い順に解決される。
-   **行動消費:** スキルや魔法、アイテム使用は1回の行動として扱い、完了後に次の行動を選択する。
-   **パーティ構成:** `前衛 (Forward)` `中衛 (Middle)` `後衛 (Rear)` の3人固定。
-   **行動決定:** 優先度順にガンビットを評価し、ターゲットに該当する対象が存在し、実行可能なアクションを選択する。
-   **フォールバック:** 実行可能なルールがない場合、`通常攻撃` を行う。
-   **v1実装メモ:** 各行動は完了までに時間がかかるタスクとして扱われ、味方/敵が並行して進行する。

### 2.3 ガンビット（戦法）
戦闘行動は、優先順位の高いルールから評価されるif-then形式の「ガンビット」によって決定される。

-   **ルール構成:** `ガンビット = ターゲット (Target) + アクション (Action)`
-   **評価タイミング:** 各キャラクターの行動決定時に上から順に評価する。
-   **編集:** ルールの追加/削除/並び替えに対応する。
-   **上限:** 1キャラクターあたり最大16行。将来的に増減する可能性がある。
-   **ターゲットの役割:** ターゲットは対象の絞り込み条件を含む（例: `味方のHP < 50%`, `HPが最も低い敵`）。

-   **v1 ガンビットリスト（例）:**
| ターゲット (Target) | アクション (Action) | 優先度 |
| :--- | :--- | :--- |
| 味方のHP < 25% | `大回復魔法` | 1 (最高) |
| 味方のHP < 50% | `小回復魔法` | 2 |
| 自分自身 | `防御する` | 3 |
...
#### 2.3.1 ターゲット (Target) の種類
-   自分自身
-   味方 (例: `HPが75%以下の味方`, `HPが最も低い味方`, `味方全体`)
-   敵 (例: `HPが最も低い敵`, `先頭の敵`, `自分を攻撃している敵`, `「非定型」の敵`, `攻撃予兆の敵`)
-   **v1実装済み:**
    -   自分自身
    -   ランダムな味方/敵
    -   味方全体/敵全体
...
#### 2.3.3 ジョブ別デフォルトガンビット (v1)
-   キャラクター作成時に、ジョブごとの初期戦闘ルールを設定する。
-   初期ルールは編集・削除可能。
-   既存セーブのキャラクターには遡って適用しない。

**Fighter (戦士)**
| 優先度 | ターゲット | アクション |
| :--- | :--- | :--- |
| 1 | HPが25%以下の味方 | 回復薬を使う |
| 2 | HPが最も低い敵 | 物理攻撃 |
...
6.  **ガンビット編集画面:** ターゲット・アクションを選択し、ルールの追加・削除・並べ替えを行う。
7.  **ダンジョン選択画面:** 到達済みフロアの一覧から探索開始フロアを選ぶ。
8.  **ダンジョン探索画面:**
    -   パーティメンバーの簡易ステータス表示 (前/中/後・アイコン・HP/MP)。
    -   探索/行動を統合した時系列ログの表示（行動は完了時にログ化、進行中の行動は%と横バーで進捗を表示）。
    -   フロア/戦闘回数は1行表示。
    -   操作: `Pause/Play` (探索の一時停止/再開), `Camp` (キャンプ), `Return to City` (街へ帰還)。
    -   `Camp`/`Return to City` は確認ダイアログを表示する。
...
5.  **キャラクター詳細画面**
    ...
    -   **状態:** 選択中装備/アイテム。
6.  **ガンビット編集画面**
    -   **コンポーネント:** ルール一覧 (最大16行)、`追加`/`削除`/`並び替え`、ターゲット/アクション選択UI。
    -   **状態:** 編集中ルール、選択中ターゲット/アクション。
7.  **ショップ画面**
...
8.  **ダンジョン選択画面**
    ...
    -   **状態:** 選択中フロア。
9.  **ダンジョン探索画面**
    -   **コンポーネント:** パーティ簡易ステータス(前/中/後 + アイコン + HP/MP)、戦闘中の敵簡易ステータス(アイコン + HP/MP + 名前、横スクロール)、探索/行動の統合タイムライン（進行中の行動は%と横バーで進捗を表示）、フロア/戦闘回数の1行表示、`Pause/Play`、`Camp`、`Return to City`。
